...
# useradd -G sudo -m ${client_name} -s /bin/bash
# passwd ${client_name}
port="443"
protocol="1"

# make sure we allow connections on port 443/udp
sudo firewall-cmd --zone=trusted --permanent --add-port=$port/udp
sudo firewall-cmd --zone=public --permanent --add-port=$port/udp
sudo firewall-cmd --reload

### BEGIN OPENVPN INSTALL
...
if grep -qs "ubuntu" /etc/os-release; then
	...
	echo "It looks like you're on an unsupported distribution.
...
if [[ ! -e /etc/openvpn/server/server.conf ]]; then
	...
	echo
	echo "Enter port for openvpn: [$port]"
	# port="443"
...
verb 3" > /etc/openvpn/server/client-common.txt
	...
	echo
	echo
	echo "Client configuration created in: " ~/"$client.ovpn"
else
	# clear
  ...
  option="$1"
	...
	case "$option" in
		1)
			echo
			echo "Enter a username for this client: [$client_name]"
			# read -p "Name: " client_name
			client=$(sed 's/[^0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_-]/_/g' <<< "$client_name")
			while [[ -z "$client" || -e /etc/openvpn/server/easy-rsa/pki/issued/"$client".crt ]]; do
				echo "$client: invalid name."
				read -p "Enter a username for this client: " client_name
				client=$(sed 's/[^0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_-]/_/g' <<< "$client_name")
			done
			cd /etc/openvpn/server/easy-rsa/
			EASYRSA_CERT_EXPIRE=3650 ./easyrsa build-client-full "$client" nopass
			...
			make_client
			echo
			echo "$client added. Configuration created in: ~/${client}.ovpn"
			exit
		;;
		2)
		...
		3)
			echo
			read -p "Confirm removing OpenVPN? [y/N]: " remove
...
ip=$(curl ifconfig.me)
echo "Download the client profile like this!"
echo "scp root@$ip:~/${client_name}.ovpn ."
